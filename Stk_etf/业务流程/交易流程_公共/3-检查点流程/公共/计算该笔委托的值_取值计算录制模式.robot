...
接口/表取值流程_取值计算录制模式_备份
    ...
    FOR    ${i}    IN    @{变化数据名称}
        ...
        ${字段计算差值_接口}    ${字段无需计算_接口}    Run Keyword If    '${i}' in ('可用资金查询_10303001','可用股份查询_10303002')    提取接口字段_取值录制模式    ${i}    ${返回信息}
        LOG    -----表
        ${字段计算差值_表}    ${字段无需计算_表}    Run Keyword If    '${i}' in ('CUACCT_FUND','STK_ASSET')    提取表字段_取值录制模式    ${i}
        LOG    -----转换
        ${计算}    Set Variable IF    '${i}' in ('可用资金查询_10303001','可用股份查询_10303002')    ${字段计算差值_接口}    ${字段计算差值_表}
        ${无计算}    Set Variable IF    '${i}' in ('可用资金查询_10303001','可用股份查询_10303002')    ${字段无需计算_接口}    ${字段无需计算_表}
...
接口/表取值流程_取值计算录制模式
    ...
    FOR    ${i}    IN    @{变化数据名称}
        ...
        LOG    -----接口    #ReqFundQuery_资金查询    ReqShareQuery_股份查询    basedb.t_fund_assets    basedb.t_stocks
        ${返回码}    ${返回信息}    Run Keyword If    '${i}' in ('可用资金查询_10303001','可用股份查询_10303002')    ${i}
        ${字段计算差值_接口}    ${字段无需计算_接口}    Run Keyword If    ${返回信息} != [] and '${i}' in ('可用资金查询_10303001','可用股份查询_10303002')    提取接口字段_取值录制模式    ${i}    ${返回信息}
        ...    ELSE IF    ${返回信息} == [] and '${i}' in ('可用资金查询_10303001','可用股份查询_10303002')    提取接口字段_取值录制模式中接口返回空数据处理    ${i}    ${返回信息}
        LOG    -----表
        ${字段计算差值_表}    ${字段无需计算_表}    Run Keyword If    '${i}' in ('CUACCT_FUND','STK_ASSET')    提取表字段_取值录制模式    ${i}
        LOG    -----转换
        ${计算}    Set Variable IF    '${i}' in ('可用资金查询_10303001','可用股份查询_10303002')    ${字段计算差值_接口}    ${字段计算差值_表}
...
数据写入表_录制模式
    ...
    ...    ELSE    Create List    ${TEST_NAME}${number}    ${客户代码_全局变量}    ${接口数据名称}    ${接口数据}    ;    #用例编号    #客户号
    log    ${list}
    ${ret}    Run Keyword And Continue On Failure    Call Stored Function    DDXT_RFTEST_DATA_LOG    ${list}    #写入表：DDXT_RFAUTOTEST_DATA_LOG    #多次执行记录的表DDXT_TEST_RFAUTOTEST_DATA_LOG
    ${ret}    Set Variable If    '${ret}'=='None'    2    ${ret}
    [Return]    ${ret}
