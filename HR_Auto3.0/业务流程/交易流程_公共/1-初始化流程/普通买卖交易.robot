...
Resource          ../../交易流程_华锐/1-初始化流程/普通买卖交易.robot
Resource          ../3-检查点流程/公共/计算该笔委托的值_取值计算录制模式.robot
Resource          ../../../公共目录/配置信息/华锐数据库查询.robot
...
普通买卖交易初始化流程
    ...
    Run KeyWord If    '${业务_全局变量}' in ('181','182','187','188','825','826') or ${委托数量_全局变量}==0    ETF申赎数量查询
    普通买卖初始化

普通买卖初始化
    [Arguments]    ${调整资金}=${资金初始化}    ${调整股份}=${股份初始化}    ${权限}=${Empty}    ${接口类型}=${接口类型_全局变量}
    ${变化数据_华锐}    Run Keyword If    '${接口类型}'=='Cash'    华锐_普通买卖初始化    ${调整资金}    ${调整股份}
    ${变化数据}    Run Keyword If    '${接口类型}'=='Cash'    Set Variable    ${变化数据_华锐}
...
新股申购初始化
    ...
    ${变化数据_华锐}    Run Keyword If    '${接口类型}'=='Cash'    华锐_新股申购初始化
    ${变化数据}    Run Keyword If    '${接口类型}'=='Cash'    Set Variable    ${变化数据_华锐}
    ${字段提取_需计算_委托前_全局变量}    ${字段提取_无需计算_委托前}    接口/表取值流程_取值计算录制模式    ${变化数据}
    Set Global Variable    ${字段提取_需计算_委托前_全局变量}
    log    ${字段提取_需计算_委托前_全局变量}
