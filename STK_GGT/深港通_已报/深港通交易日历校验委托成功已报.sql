...
SELECT top 1 '交易日深市港股买入成功' AS name,'订单成功' AS result,* FROM CTE_Master_130_140  where STKBD IN (SELECT STKBD FROM STK_CALENDAR WHERE PHYSICAL_DATE = @date AND STKBD='03' AND TRD_DATE_FLAG=1) AND CUACCT_CODE NOT IN ('11')
union
SELECT top 1 '交易日深市港股卖出成功' AS name,'订单成功' AS result,* FROM CTE_Master_131_140  where STKBD IN (SELECT STKBD FROM STK_CALENDAR WHERE PHYSICAL_DATE = @date AND STKBD='03' AND TRD_DATE_FLAG=1) AND CUACCT_CODE NOT IN ('11')