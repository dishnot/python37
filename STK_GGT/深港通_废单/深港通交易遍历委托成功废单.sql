...
--深港通买入全部使用主资金CUACCT_FUND_DETAIL_FUND_AVL,CUACCT_FUND_BORROW-FUND_BORROW_BLN
...
UNION
SELECT TOP 1 '卖出全部归还主资金' AS name,'订单成功' AS result,A.CUST_CODE,A.CUACCT_CODE,A.STKBD,A.TRDACCT,A.STK_BIZ,A.STK_BIZ_ACTION,A.STK_CODE,A.STK_CLS,
    CASE WHEN round(FUND_BORROW_BLN/1000.0/A.ORDER_PRICE/A.ORDER_QTY,0) <  1 THEN  round(FUND_BORROW_BLN/1000.0/ORDER_PRICE/A.ORDER_QTY,0)*A.ORDER_QTY
    ELSE A.ORDER_QTY END AS ORDER_QTY,A.ORDER_PRICE,A.SECURITY_LEVEL,B.FUND_BORROW_BLN/1000.0 FROM CTE_Master_131_100 A LEFT JOIN  (SELECT FUND_BORROW_BLN,CUACCT_CODE   FROM CUACCT_FUND_BORROW) B ON A.CUACCT_CODE = B.CUACCT_CODE WHERE ORDER_QTY*ORDER_PRICE<=FUND_BORROW_BLN/1000.0 AND A.CUACCT_CODE NOT IN ('11')
UNION
SELECT TOP 1 '卖出全部归还二级台账资金' AS name,'订单成功' AS result ,A.CUST_CODE,A.CUACCT_CODE,A.STKBD,A.TRDACCT,A.STK_BIZ,A.STK_BIZ_ACTION,A.STK_CODE,A.STK_CLS,A.ORDER_QTY,A.ORDER_PRICE,A.SECURITY_LEVEL,B.FUND_BORROW_BLN/1000.0 FROM CTE_Master_131_100 A LEFT JOIN  (SELECT FUND_BORROW_BLN,CUACCT_CODE   FROM CUACCT_FUND_BORROW) B ON A.CUACCT_CODE = B.CUACCT_CODE WHERE FUND_BORROW_BLN=0 or A.CUACCT_CODE NOT IN (SELECT CUACCT_CODE FROM CUACCT_FUND_BORROW ) AND A.CUACCT_CODE NOT IN ('11')
UNION
SELECT TOP 1 '卖出部分归还主资金' AS name,'订单成功' AS result,A.CUST_CODE,A.CUACCT_CODE,A.STKBD,A.TRDACCT,A.STK_BIZ,A.STK_BIZ_ACTION,A.STK_CODE,A.STK_CLS,
    CASE WHEN round(FUND_BORROW_BLN/1000.0/A.ORDER_PRICE/A.ORDER_QTY,0) >  1 THEN  round(FUND_BORROW_BLN/1000.0/ORDER_PRICE/A.ORDER_QTY,0)*A.ORDER_QTY
    ELSE A.ORDER_QTY END AS ORDER_QTY,A.ORDER_PRICE,A.SECURITY_LEVEL,B.FUND_BORROW_BLN/1000.0 FROM CTE_Master_131_100 A LEFT JOIN  (SELECT FUND_BORROW_BLN,CUACCT_CODE   FROM CUACCT_FUND_BORROW) B ON A.CUACCT_CODE = B.CUACCT_CODE WHERE ORDER_QTY*ORDER_PRICE>FUND_BORROW_BLN/1000.0 AND  A.CUACCT_CODE NOT IN ('11')