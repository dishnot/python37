...
查询接口与表比对的通用流程
    ...
    log    ${调用接口}
    FOR    ${i}    IN    @{调用接口}
        ${返回码}    ${返回信息}    Run Keyword If    '${i}'in ('当日委托查询_10303003','ReqOrderQuery_订单查询')    委托撤单查询_多条数据_比对录制模式    ${i}
        ...    ELSE    ${i}
        ${返回码数据类型}    Evaluate    isinstance($返回码, str)
        log    ${返回码数据类型}
        ${返回码替换值}    Create List
...
委托撤单查询_多条数据_比对录制模式
    ...
    FOR    ${i}    IN    @{委托_撤单合同序号Values}
        ...
        log    ${返回信息}
        Run Keyword If    '${i}'!='${EMPTY}' and '${i}'!='0'    Append To List    ${合并信息}    ${返回信息[0]}
        log    ${合并信息}
    END
    log    ${合并信息}
    [Return]    ${返回码}    ${合并信息}
