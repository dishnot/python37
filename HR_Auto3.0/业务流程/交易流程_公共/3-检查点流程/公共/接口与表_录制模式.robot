...
Resource          ../../../../公共目录/配置信息/华锐_字段信息.robot
...
查询接口与表取值录制的通用流程
    ...
    FOR    ${i}    IN    @{数据名称}
        ${返回码}    ${返回信息}    Run Keyword If    '${i}'in ('ReqOrderQuery_订单查询')    委托撤单查询_多条数据_比对录制模式    ${i}
        ...    ELSE IF    '${i}'in ('ReqTradeOrderQuery_成交查询','ReqTradeOrderExQuery_增强成交查询','ReqETFQueryOrderEx_ETF申赎成交查询')    ${i}
        ${返回信息_条数}    Run Keyword If    '${i}' in ('ReqOrderQuery_订单查询','ReqTradeOrderQuery_成交查询','ReqTradeOrderExQuery_增强成交查询','ReqETFQueryOrderEx_ETF申赎成交查询')    Get Length    ${返回信息}
        ...    ELSE    Set Variable    1
        LOG    ${返回信息}
        ${返回信息}    Run KeyWord If    '${业务_全局变量}' in ('181','182','187','188','827','828') and '${i}'=='ReqETFQueryOrderEx_ETF申赎成交查询'and '${交易板块_全局变量}'=='00'    深市ETF申赎成分股回报数据处理    ${返回信息}
        ...    ELSE    Set Variable    ${返回信息}
        ${排序字段}    Run Keyword If    '${业务_全局变量}' not in ('181','182','187','188','827','828')    Set Variable    security_id
        ...    ELSE    Set Variable    order_array|security_id
        ${返回信息}    Run Keyword If    '${i}' in ('ReqOrderQuery_订单查询','ReqTradeOrderQuery_成交查询','ReqTradeOrderExQuery_增强成交查询','ReqETFQueryOrderEx_ETF申赎成交查询') and '${返回信息_条数}'>'0'    接口数据排序    ${返回信息}    ${排序字段}
        ...    ELSE    Set Variable    ${返回信息}
        log    ${返回信息}
...
提取接口字段_取值录制模式
    ...
    ${委托字段}    Run Keyword If    '${接口类型}'=='Cash'    华锐_录制字段    ${接口名称}
...
接口与表处理_录制模式
    ...
    [Documentation]    1.20211123 chm 新增
    ${委托字段}    Run Keyword If    '${接口类型}'=='Cash'    华锐_录制字段    ${数据名称}
    log    ---接口数据处理---
    ${字段key}    ${字段values}    字段与值建立成字典_取值计算录制模式    ${委托字段}
    ${接口返回信息}    Run Keyword If    '${数据名称}' in ('ReqOrderQuery_订单查询','ReqTradeOrderQuery_成交查询','ReqTradeOrderExQuery_增强成交查询','ReqETFQueryOrderEx_ETF申赎成交查询')    查询接口与表数据进行格式处理_录制模式    ${返回信息}    ${字段key}    YES
    log    ---表数据---
    ${查询表结果}    Run Keyword If    '${数据名称}' in ('tradingdb.t_order','tradingdb.t_report','tradingdb.t_report_etf_component')    买卖交易委托后检查_表查询    ${数据名称}    ${字段key}
    ${len}    Run Keyword If    '${数据名称}' in ('tradingdb.t_order','tradingdb.t_report','tradingdb.t_report_etf_component')    Get Length    ${查询表结果}
    ${表结果处理}    Run Keyword If    '${数据名称}' in ('tradingdb.t_order','tradingdb.t_report','tradingdb.t_report_etf_component')    查询接口与表数据进行格式处理_录制模式    ${查询表结果}
    ${表结果}    Run Keyword If    '${len}'>'0'    Set Variable    ${表结果处理}
    ...    ELSE IF    '${len}'=='0'    Set Variable    无记录;
    log    ${表结果}
    ${数据处理结果}    Set Variable If    '${数据名称}' in ('ReqOrderQuery_订单查询','ReqTradeOrderQuery_成交查询','ReqTradeOrderExQuery_增强成交查询','ReqETFQueryOrderEx_ETF申赎成交查询')    ${接口返回信息}    ${表结果}
    log    ${数据处理结果}
    [Return]    ${数据处理结果}
...
接口与表处理_比对模式
    ...
    [Documentation]    1.20211123 chm 新增
    ${接口字典}    Run Keyword If    '${接口类型}'=='Cash'    华锐_比对字段    ${查询接口名称}
    ${接口结果}    Create List
    log    ---接口字段重新提取---
    FOR    ${i}    IN    @{返回信息}
        ${接口字段}    字典重新提取成新字典    ${i}    ${接口字典}
        Append To List    ${接口结果}    ${接口字段}
        Exit For Loop If    '${查询接口名称}'=='55555'
    END
    log    ${接口结果}
    log    ---表字段提取并转换为字典---
