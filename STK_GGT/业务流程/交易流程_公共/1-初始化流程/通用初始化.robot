...
客户与证券信息查询
    ...
    ${委托当前价格}    EVALUATE    round(${委托当前价格},3)    #价格保留3位小数
    ${委托涨停价格}    Get From LIST    ${委托涨停价格[0]}    0
    #${委托涨停价格}    Convert To Integer    ${委托涨停价格}
    log    ${委托涨停价格}
    价差    ${委托当前价格}
    ${当前价加价差}    Evaluate    round(${委托当前价格}+${价差},3)
    ${委托价格}    Run Keyword If    ${交易信息["委托价格"]}!=0    Set Variable    ${交易信息["委托价格"]}
    ...    ELSE IF    ${交易信息["委托价格"]}==0 and ${委托涨停价格}!=0 and ${委托涨停价格}!=[] and ${委托涨停价格}<100000 and ${委托当前价格}==0    Set Variable    ${委托涨停价格}
    ...    ELSE    Set Variable    ${当前价加价差}
    log    ${委托价格}
    Run Keyword If    '${接口类型}'=='Stk'    金证_客户与证券信息查询
    #委托价格_设置全局变量    ${委托价格整数}
    ${委托价格_全局变量}    Run Keyword If    '${接口类型}'=='Stk'    Set Variable    ${委托价格}
