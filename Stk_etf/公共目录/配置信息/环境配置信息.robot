...
环境信息查询与连接
    ...
    ${队列信息}    Run Keyword If    '${接口类型}'=='Stk' and '${TEST_ENV}'in'(27,78)'    Set Variable    {\\"address\\":\\"${环境["数据库"]}\\",\\"sendq\\":\\"${环境["服务队列"]}\\",\\"recvq\\":\\"${环境["应答队列"]}\\",\\"servername\\":\\"${环境["servername"]}\\",\\"username\\":\\"${环境["username"]}\\",\\"password\\":\\"${环境["password"]}\\",\\"protocol\\":${环境["protocol"]},\\"port\\":${环境["port"]}}
...
POST接口下单请求_UTP数据银行
    ...
    log    ${接口中文名称}
    #根据不同系统传参不一样
    ${接口入参}    Run Keyword If    '${接口中文名称}'!='${EMPTY}' and '${接口类型}'=='Stk'    Set Variable    {"env": "${环境}","func_code": "${接口}","bus_type": "${接口类型}","bus_name": "${接口中文名称}","version": "${接口版本}","sync_flag": "${sync_flag}","data": {${接口参数}}}
