...
WHERE A.CUACCT_CODE = C.CUACCT_CODE AND A.STK_CODE=C.STK_CODE AND A.STKBD = B.STKBD AND A.STK_CLS=B.STK_CLS AND B.STKBD=C.STKBD AND B.STK_CLS=C.STK_CLS AND C.STK_AVL>ORDER_QTY AND
 B.STK_CLS = 'A'  AND A.PRICE BETWEEN B.BGN_PRICE/10000 AND B.END_PRICE/10000   --价格+价差逻辑处理                           
)

--深港通标的范围委托校验

SELECT top 1 '港股通买入委托价差价位的整数倍，委托正常' AS name,'订单成功' AS result,* FROM CTE_Master_130_140   WHERE CUACCT_CODE NOT IN ('11')
UNION
SELECT top 1 '港股通卖出委托价差价位的整数倍，委托正常' AS name,'订单成功' AS result,* FROM CTE_Master_131_140  WHERE  CUACCT_CODE NOT IN ('11')