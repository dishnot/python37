...
SELECT top 1 '港股通买入标的范围内的深港通股票成功' AS name,'订单成功' AS result,* FROM CTE_Master_130_140  where STK_CODE IN (SELECT STK_CODE FROM HGT_STK_TRD_STATUS WHERE  STKBD='03' AND OBJECT_FLAG=1)  AND CUACCT_CODE NOT IN ('11')
UNION
SELECT top 1 '港股通卖出标的范围内的深港通股票成功' AS name,'订单成功' AS result,* FROM CTE_Master_131_140  where STK_CODE IN (SELECT STK_CODE FROM HGT_STK_TRD_STATUS WHERE  STKBD='03' AND OBJECT_FLAG=1)  AND CUACCT_CODE NOT IN ('11')
UNION
SELECT top 1 '港股通卖出标的范围外的深港通股票成功' AS name,'订单成功' AS result,* FROM CTE_Master_131_140  where STK_CODE  IN (SELECT STK_CODE FROM HGT_STK_TRD_STATUS WHERE  STKBD='03' AND OBJECT_FLAG=0) AND CUACCT_CODE NOT IN ('11')