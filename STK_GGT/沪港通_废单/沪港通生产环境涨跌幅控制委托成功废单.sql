...
(A.PRICE+ 24*ROUND(B.STK_SPREAD/10000.000,3,1)) AS AA_PRICE
FROM CTE_TJ2  A,  STK_PRICE_UNIT B , STK_ASSET C    
WHERE A.CUACCT_CODE = C.CUACCT_CODE AND A.STK_CODE=C.STK_CODE AND A.STKBD = B.STKBD AND A.STK_CLS=B.STK_CLS AND B.STKBD=C.STKBD AND B.STK_CLS=C.STK_CLS AND C.STK_AVL>ORDER_QTY AND B.STK_CLS = 'A' AND  A.STKBD = B.STKBD AND A.PRICE BETWEEN B.BGN_PRICE/10000 AND B.END_PRICE/10000   --价格+价差逻辑处理                           
)

--港股通生产环境涨跌幅控制

SELECT top 1 '高于最低买入委托价' AS name,'订单成功' AS result,* FROM CTE_Master_130_140_24 WHERE ORDER_PRICE>AA_PRICE
UNION
SELECT top 1 '低于最高买入委托价' AS name,'订单成功' AS result,* FROM CTE_Master_130_140_9  WHERE ORDER_PRICE <AA_PRICE
UNION
SELECT top 1 '高于最低卖出委托价' AS name,'订单成功' AS result,* FROM CTE_Master_131_140_9 WHERE ORDER_PRICE>AA_PRICE
UNION
SELECT top 1 '低于最高卖出委托价' AS name,'订单成功' AS result,* FROM CTE_Master_131_140_24  WHERE ORDER_PRICE <AA_PRICE