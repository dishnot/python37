...
Resource          ../../公共目录/通用操作/sqltest关键字.robot
Library           Collections
...
港股通交易委托流程
    ...
    环境信息查询与连接
    log    ---初始化
    普通买卖交易初始化流程    ${交易信息}    ${资金初始化}    ${股份初始化}    ${委托数量}    ${权限}
    log    ---委托
    普通买卖下单流程
    log    ---检查
    普通买卖交易检查流程
...
sqltest港股通交易委托流程
    ...
    FOR    ${i}    IN    @{sqltest查询数据}
        ${用例}    Set Variable If    '${i}[name]'=='${用例名}'    ${i}
        Exit For Loop If    '${i}[name]'=='${用例名}'
        LOG    ${i}
    #${b}    Evaluate    ${b}+1
    END
    Should be equal as strings    ${用例}[name]    ${用例名}
    sqltest委托入参数据处理    ${用例}
    sqltest普通买卖初始化
    Run KeyWord If    '${目录}'not in ('深港通委托异常','沪港通委托异常')    金证_普通买卖下单流程
    ...    ELSE    sqltest异常买卖下单流程
    普通买卖交易检查流程
