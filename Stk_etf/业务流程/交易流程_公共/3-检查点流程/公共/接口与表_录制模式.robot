...
查询接口与表取值录制的通用流程
    ...
    FOR    ${i}    IN    @{数据名称}
        ...
        ...    ELSE IF    '${接口类型}'=='Cash'    Set Variable    security_id
...
查询接口与表全录制的通用流程
    ...
    FOR    ${i}    IN    @{数据名称}
        ${名称}    Set Variable    ${i}
        ${i}    Run Keyword If    '${i}'=='可用资金_10303001_全接口字段'    Set Variable    可用资金查询_10303001
        ...    ELSE IF    '${i}'=='可用股份_10303002_全接口字段'    Set Variable    可用股份查询_10303002
        ...    ELSE    Set Variable    ${i}
        ${返回码}    ${返回信息}    Run Keyword If    '${i}'in ('可用资金查询_10303001','可用股份查询_10303002')    ${i}
        ${返回信息_条数}    Run Keyword If    '${i}' in ('可用资金查询_10303001','可用股份查询_10303002')    Get Length    ${返回信息}
        ...    ELSE    Set Variable    1
        ${排序字段}    Run Keyword If    '${i}' in ('可用资金查询_10303001')    Set Variable    CUACCT_CODE
        ...    ELSE IF    '${i}' in ('可用股份查询_10303002')    Set Variable    STK_CODE
        ${返回信息}    Run Keyword If    '${i}' in ('可用资金查询_10303001','可用股份查询_10303002') and '${返回信息_条数}'>'0'    接口数据排序    ${返回信息}    ${排序字段}
        ...    ELSE    Set Variable    ${返回信息}
        log    ${返回信息}
        LOG    ${i}
        ${i}    Set Variable    ${名称}
        ${数据处理结果}    Run Keyword If    '${返回信息_条数}'>'0'    总对总接口与表处理_录制模式    ${i}    ${返回信息}
        ...    ELSE IF    '${返回信息_条数}'=='0'    Set Variable    无查询结果;
        comment    #接口数据写入表中
        ${比对结果}    数据写入表_录制模式    ${i}    ${数据处理结果}
        ${执行结果}    Run Keyword If    '${比对结果}'=='0'    Set Variable    校验通过
...
接口与表处理_录制模式
    ...
    log    ---表数据---
    ${查询表结果}    Run Keyword If    '${数据名称}' in ('STK_ORDER','STK_MATCHING','STK_ORDER_DETAIL')    买卖交易委托后检查_表查询    ${数据名称}    ${字段key}
    ${len}    Run Keyword If    '${数据名称}' in ('STK_ORDER','STK_MATCHING','STK_ORDER_DETAIL')    Get Length    ${查询表结果}
    ${表结果处理}    Run Keyword If    '${数据名称}' in ('STK_ORDER','STK_MATCHING','STK_ORDER_DETAIL')    查询接口与表数据进行格式处理_录制模式    ${查询表结果}
...
总对总接口与表处理_录制模式
    ...
    log    ---接口数据处理---
    ${字段key}    ${字段values}    字段与值建立成字典_取值计算录制模式    ${委托字段}
    ${接口返回信息}    Run Keyword If    '${数据名称}' in ('可用资金_10303001_全接口字段','可用股份_10303002_全接口字段')    查询接口与表数据进行格式处理_录制模式    ${返回信息}    ${字段key}    YES
    log    ---表数据---
    ${查询表结果}    Run Keyword If    '${数据名称}' in ('CUACCT_FUND全表字段','STK_ASSET全表字段')    总对总检查_表查询    ${数据名称}    ${委托字段}
    ${len}    Run Keyword If    '${数据名称}' in ('CUACCT_FUND全表字段','STK_ASSET全表字段')    Get Length    ${查询表结果}
    ${表结果处理}    Run Keyword If    '${数据名称}' in ('CUACCT_FUND全表字段','STK_ASSET全表字段')    查询接口与表数据进行格式处理_录制模式    ${查询表结果}
    ${表结果}    Run Keyword If    '${len}'>'0'    Set Variable    ${表结果处理}
    ...    ELSE IF    '${len}'=='0'    Set Variable    无记录;
    log    ${表结果}
    ${数据处理结果}    Set Variable If    '${数据名称}' in ('可用资金_10303001_全接口字段','可用股份_10303002_全接口字段')    ${接口返回信息}    ${表结果}
    log    ${数据处理结果}
    [Return]    ${数据处理结果}
...
查询接口与表委托前全录制的通用
    ...
    FOR    ${i}    IN    @{数据名称}
        ${返回码}    ${返回信息}    Run Keyword If    '${i}'=='委托前_可用资金查询_接口全字段'    可用资金查询_10303001
        ...    ELSE IF    '${i}'=='委托前_可用股份查询_接口全字段'    可用股份查询_10303002
        ${返回信息_条数}    RUN Keyword If    '${i}'in ('委托前_可用资金查询_接口全字段','委托前_可用股份查询_接口全字段')    Get Length    ${返回信息}
        ...    ELSE    Set Variable    1
        ${排序字段}    Run Keyword If    '${i}'=='委托前_可用资金查询_接口全字段'    Set Variable    CUACCT_CODE
        ...    ELSE IF    '${i}'=='委托前_可用股份查询_接口全字段'    Set Variable    STK_CODE
        ${返回信息}    Run Keyword If    '${i}' in ('委托前_可用资金查询_接口全字段','委托前_可用股份查询_接口全字段') and '${返回信息_条数}'>'0'    接口数据排序    ${返回信息}    ${排序字段}
        ...    ELSE    Set Variable    ${返回信息}
        log    ${返回信息}
        log    ${数据名称}
        LOG    ${i}
...
委托前总对总接口与表处理_录制模式
    ...
    [Documentation]    1.2023
    log    ${数据名称}
    ${数据名称A}    Run Keyword If    '${数据名称}'=='委托前_可用资金查询_接口全字段'    Set Variable    可用资金_10303001_全接口字段
    ...    ELSE IF    '${数据名称}'=='委托前_可用股份查询_接口全字段'    Set Variable    可用股份_10303002_全接口字段
    ...    ELSE IF    '${数据名称}'=='委托前_CUACCT_FUND_表全字段'    Set Variable    CUACCT_FUND全表字段
    ...    ELSE IF    '${数据名称}'=='委托前_STK_ASSET_表全字段'    Set Variable    STK_ASSET全表字段
    ${委托字段}    Run Keyword If    '${接口类型}'=='Stk'    金证_录制字段    ${数据名称A}
    log    ---接口数据处理---
    log    ${委托字段}
    ${字段key}    ${字段values}    总对总字段与值建立成字典_取值录制模式    ${委托字段}
    ${接口返回信息}    Run Keyword If    '${数据名称}' in ('委托前_可用资金查询_接口全字段','委托前_可用股份查询_接口全字段')    查询接口与表数据进行格式处理_录制模式    ${返回信息}    ${字段key}    YES
    log    ---表数据---
    log    ${数据名称}
    ${查询表结果}    Run Keyword If    '${数据名称}' in ('委托前_CUACCT_FUND_表全字段','委托前_STK_ASSET_表全字段')    总对总检查_表查询    ${数据名称}    ${委托字段}
    log    ${数据名称}
    ${len}    Run Keyword If    '${数据名称}' in ('委托前_CUACCT_FUND_表全字段','委托前_STK_ASSET_表全字段')    Get Length    ${查询表结果}
    log    ${数据名称}
    ${表结果处理}    Run Keyword If    '${数据名称}'in ('委托前_CUACCT_FUND_表全字段','委托前_STK_ASSET_表全字段')    查询接口与表数据进行格式处理_录制模式    ${查询表结果}
    ${表结果}    Run Keyword If    '${len}'>'0'    Set Variable    ${表结果处理}
    ...    ELSE IF    '${len}'=='0'    Set Variable    无记录;
    log    ${表结果}
    ${数据处理结果}    Set Variable If    '${数据名称}' in ('委托前_可用资金查询_接口全字段','委托前_可用股份查询_接口全字段')    ${接口返回信息}    ${表结果}
    log    ${数据处理结果}
    [Return]    ${数据处理结果}
