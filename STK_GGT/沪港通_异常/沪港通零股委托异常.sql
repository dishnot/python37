...
WITH CTE_Cuacct AS (SELECT C.USER_CODE AS CUST_CODE,  --查符合条件账户
					...
					INNER JOIN (SELECT * FROM STK_TRDACCT WHERE STKBD ='13'  AND TRDACCT_STATUS = '0' AND  TRDACCT_EXCLS in ('0','1') AND STKPBU <> ' ' 	
					) D 
...
,
CTE_Zero1_130_141 AS  --临时表定义零股买入场景
(
SELECT ID, 
STK_LWLMT_PRICE, 
STK_UPLMT_PRICE, 
...
A.STK_CODE, 
A.STK_CLS, 
FLOOR(A.STK_LOT_SIZE / 2) AS ORDER_QTY,
A.STK_LOT_SIZE,
STK_SUB_CLS, 
'0' SECURITY_LEVEL,
(A.PRICE+ ROUND(B.STK_SPREAD/10000.000,3,1)) AS ORDER_PRICE 
FROM CTE_TJ2  A,  STK_PRICE_UNIT B
WHERE A.STKBD = B.STKBD  AND B.STK_CLS = 'A'  AND A.PRICE BETWEEN B.BGN_PRICE/10000 AND B.END_PRICE/10000   --价格+价差逻辑处理                           
)
,
...
WHERE A.CUACCT_CODE = C.CUACCT_CODE AND A.STK_CODE=C.STK_CODE AND  A.STKBD = B.STKBD  AND A.STK_CLS=B.STK_CLS  AND B.STKBD=C.STKBD AND B.STK_CLS=C.STK_CLS
AND C.STK_AVL>0 AND C.STK_AVL>A.STK_LOT_SIZE  AND C.STK_AVL % A.STK_LOT_SIZE=0 AND B.STK_CLS = 'A'  AND A.PRICE BETWEEN B.BGN_PRICE/10000 AND B.END_PRICE/10000   --价格+价差逻辑处理                           
)


--沪港通委托零股失败

SELECT TOP 1 '深港通买入零股失败' AS name,A.*  FROM CTE_Zero1_130_141 A 
UNION 
SELECT TOP 1 '不存在小于港股整手数的持仓,零股卖出失败' AS name,A.*  FROM CTE_Zero2_131_141 A