...
证券信息查询_10301001
    ...
    ${货币代码_全局变量}    Run KeyWord If    '${货币代码_全局变量}'!='0'    Set Variable    0
    ...    ELSE    Set Variable    ${货币代码_全局变量}
...
港股通汇率信息查询_10301053
    [Arguments]    ${交易板块}=${交易板块_全局变量}    ${货币代码}=1
    [Documentation]    1.20230927 ch 新增
    ${resp}    POST接口下单请求_UTP数据银行    10301053    "CURRENCY": "${货币代码}","STKBD":"${交易板块}"    港股通汇率信息查询
    log    ${resp}
    ${返回码}    set variable    ${resp[0]["data"][0]}
    ${返回信息}    set variable    ${resp[0]["data"][1]}
    [Return]    ${返回码}    ${返回信息}

港股通标的证券状态查询_10301059
    [Arguments]    ${交易板块}=${交易板块_全局变量}    ${证券代码}=${证券代码_全局变量}
    [Documentation]    1.20230927 ch 新增
    ${resp}    POST接口下单请求_UTP数据银行    10301059    "STK_CODE": "${证券代码}","STKBD":"${交易板块}"    港股通标的证券状态查询
    log    ${resp}
    ${返回码}    set variable    ${resp[0]["data"][0]}
    ${返回信息}    set variable    ${resp[0]["data"][1]}
    [Return]    ${返回码}    ${返回信息}

港股通二级资金台账查询_10303027
    [Arguments]    ${资金账户}=${资金账户_全局变量}    ${货币代码}=${货币代码_全局变量}
    [Documentation]    1.20230927 ch 新增
    ${resp}    POST接口下单请求_UTP数据银行    10303027    "CUACCT_CODE": "${资金账户}","CURRENCY":"${货币代码}"    港股通二级资金台账查询
    log    ${resp}
    ${返回码}    set variable    ${resp[0]["data"][0]}
    ${返回信息}    set variable    ${resp[0]["data"][1]}
    [Return]    ${返回码}    ${返回信息}
