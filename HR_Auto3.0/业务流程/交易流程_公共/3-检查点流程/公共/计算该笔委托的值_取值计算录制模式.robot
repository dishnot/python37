...
接口/表取值流程_取值计算录制模式
    ...
    FOR    ${i}    IN    @{变化数据名称}
        log    ${i}
        LOG    -----接口    #ReqFundQuery_资金查询    ReqShareQuery_股份查询    ReqExtQueryShareEx_增强股份查询    basedb.t_fund_assets    basedb.t_stocks
        ${返回码}    ${返回信息}    Run Keyword If    '${i}' in ('ReqFundQuery_资金查询','ReqShareQuery_股份查询','ReqExtQueryShareEx_增强股份查询')    ${i}
        ${字段计算差值_接口}    ${字段无需计算_接口}    Run Keyword If    ${返回信息} != [] and '${i}' in ('ReqFundQuery_资金查询','ReqShareQuery_股份查询','ReqExtQueryShareEx_增强股份查询')    提取接口字段_取值录制模式    ${i}    ${返回信息}
        ...    ELSE IF    ${返回信息} == [] and '${i}' in ('ReqFundQuery_资金查询','ReqShareQuery_股份查询','ReqExtQueryShareEx_增强股份查询')    提取接口字段_取值录制模式中接口返回空数据处理    ${i}    ${返回信息}
        LOG    -----表
        ${字段计算差值_表}    ${字段无需计算_表}    Run Keyword If    '${i}' in ('basedb.t_fund_assets','basedb.t_stocks')    提取表字段_取值录制模式    ${i}
        LOG    -----转换
        ${计算}    Set Variable IF    '${i}' in ('ReqFundQuery_资金查询','ReqShareQuery_股份查询','ReqExtQueryShareEx_增强股份查询')    ${字段计算差值_接口}    ${字段计算差值_表}
        ${无计算}    Set Variable IF    '${i}' in ('ReqFundQuery_资金查询','ReqShareQuery_股份查询','ReqExtQueryShareEx_增强股份查询')    ${字段无需计算_接口}    ${字段无需计算_表}
...
数据写入表_录制模式
    ...
    [Documentation]    1.20211123 chm 新增
    数据库链接    ${EMPTY}
    ${list}    Run KeyWord If    '${行为_全局变量}'!='121'    Create List    ${TEST_NAME}    ${客户代码_全局变量}    ${接口数据名称}    ${接口数据}    ;
    ...    ELSE    Create List    ${TEST_NAME}${number}    ${客户代码_全局变量}    ${接口数据名称}    ${接口数据}    ;
    log    ${list}
