...
环境信息查询与连接
    ...
    ${域名/IP}    Set Variable    ${环境["服务器IP"]}
    ${端口}    Set Variable    ${环境["服务器端口"]}
    ${数据库信息}    Set Variable    Server=${环境["数据库"]};Port=${环境["数据库端口"]};Database=${环境["数据库名称"]};UID=${环境["数据库用户名称"]};PWD=${环境["数据库密码"]}
    ${队列信息}    Run Keyword If    '${TEST_ENV}'in'(118,78,136)'    Set Variable    {\\"address\\":\\"${环境["数据库"]}\\",\\"sendq\\":\\"${环境["服务队列"]}\\",\\"recvq\\":\\"${环境["应答队列"]}\\",\\"servername\\":\\"${环境["servername"]}\\",\\"username\\":\\"${环境["username"]}\\",\\"password\\":\\"${环境["password"]}\\",\\"protocol\\":${环境["protocol"]},\\"port\\":${环境["port"]}}
    ...    ELSE IF    '${TEST_ENV}'in'(47,66,67,209)'    Set Variable    {\\"address\\":\\"${环境["服务器IP"]}\\",\\"sendq\\":\\"${环境["服务队列"]}\\",\\"recvq\\":\\"${环境["应答队列"]}\\",\\"servername\\":\\"${环境["servername"]}\\",\\"username\\":\\"${环境["username"]}\\",\\"password\\":\\"${环境["password"]}\\",\\"protocol\\":${环境["protocol"]},\\"port\\":${环境["port"]}}
    ${请求信息}    Set Variable    \\"F_SESSION\\":\\"1839893105657921\\",\\"F_OP_SITE\\":\\"00505680bbd7\\",\\"F_CHANNEL\\":\\"0\\",\\"F_OP_USER\\":\\"w8sBypob9KM=\\",\\"F_OP_ROLE\\":\\"2\\",\\"F_OP_ORG\\":\\"8001\\",
    ${环境_全局变量}    Set Variable    ${环境}
    Set Global Variable    ${环境_全局变量}
    ${取值按钮}    Run KeyWord If    '${用例首字段}' in ('13','03')    Set Variable    ${取值按钮}
    ...    ELSE    Set Variable    ${int}
    Set Global Variable    ${取值按钮}
    #把数据信息存在到UTP平台上
...
POST接口下单请求_UTP数据银行
    ...
    #创建session
    ${hearder}    Run Keyword IF    '${接口中文名称}'!='用户登录API'    create dictionary    Content-Type=application/json    session=${session_全局变量}
    ...    ELSE    create dictionary    Content-Type=application/json
    create session    自定义POST请求    ${API地址["网址"]}    ${hearder}
    #接口调用
    log    ${接口中文名称}
    #根据不同系统传参不一样
    ${接口入参}    Run Keyword If    '${接口中文名称}'!='${EMPTY}'    Set Variable    {"env": "${环境}","func_code": "${接口}","bus_type": "${接口类型}","bus_name": "${接口中文名称}","version": "${接口版本}","sync_flag": "${sync_flag}","data": {${接口参数}}}
    ...
    log    ${接口入参}
    #选择后缀地址
    ${后缀}    Run Keyword If    '${接口中文名称}'!='${EMPTY}'    set variable    ${API地址["金证api接口后缀"]}
...
POST接口下单请求_UTP数据银行_备份
    ...
    log    ${接口中文名称}
    #根据不同系统传参不一样
    ${接口入参}    Run Keyword If    '${接口中文名称}'!='${EMPTY}'    Set Variable    {"env": "${环境}","func_code": "${接口}","bus_type": "${接口类型}","bus_name": "${接口中文名称}","version": "${接口版本}","data": {${接口参数}}}
    ...    ELSE IF    '${接口中文名称}'=='${EMPTY}'    Set Variable    {"env": "${环境}","func_code": "${接口}","data": [{${接口参数}}]}
    log    ${接口入参}
    #格式转换
    ${接口入参}    evaluate    demjson.encode(${接口入参})    demjson
...
数据库链接
    [Arguments]    ${接口类型}=${SPACE}    ${数据库信息}=${数据库信息_全局变量}
    #数据库链接
    #SQL
    ${xt}    Evaluate    sys.platform    sys
